...
Forgejo is the canonical SCM provider for this repository. GitHub remains a push mirror and may still host HACS-facing workflows, but Jenkins status, release, and pull-request metadata are resolved from `siczb/homeassistant_custom_dishwasher_card` in Forgejo.

The Jenkinsfile is a declarative entry into the central `homeassistant-card` profile. The Shared Library owns checkout, lifecycle, status publication, card quality gates, release handling, cleanup, and the final Home Assistant notification.
...
`.repository-documentation.yml` maps source, package, HACS, release, and CI paths to the documentation that must be assessed. A mapped source change without a matching documentation change fails before project quality gates run. A no-impact declaration is rejected when a mapping is triggered.
